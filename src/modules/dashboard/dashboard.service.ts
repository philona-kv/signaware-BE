...
export class DashboardService {
  ...
  async getDashboardData(userId: string, query: UserDashboardQueryDto): Promise<DashboardResponseDto> {
  ...
  async calculateDashboardStats(userId: string): Promise<DashboardStatsDto> {
    ...
    const totalRiskScore = completedDocuments.reduce((sum, doc) => {
      return sum + (doc.analysis?.riskScore || 0);
    }, 0);
    const avgRiskScore = totalAnalyses > 0 ? Math.round((totalRiskScore / totalAnalyses) * 10) / 10 : 0;

    // Calculate documents this month
    const now = new Date();
    const startOfMonth = new Date(now.getFullYear(), now.getMonth(), 1);
    const thisMonthDocuments = completedDocuments.filter(
      doc => doc.processingCompletedAt && doc.processingCompletedAt >= startOfMonth
    );
  ...
  async getRecentDocuments(userId: string, limit: number): Promise<RecentDocumentDto[]> {
    ...
    return documents.map(doc => ({
      ...
      title: doc.title,
      type: doc.type,
      riskScore: Math.round((doc.analysis?.riskScore || 0) * 10) / 10,
  ...
  async getMonthlyAnalytics(userId: string, monthsBack = 6): Promise<MonthlyAnalyticsDto[]> {
    ...
    documents.forEach(doc => {
      if (doc.processingCompletedAt) {
        ...
        const existing = monthlyData.get(monthKey) || { count: 0, totalRisk: 0 };
        existing.count++;
        existing.totalRisk += doc.analysis?.riskScore || 0;
    ...
    for (const [month, data] of monthlyData.entries()) {
      analytics.push({
        month,
        count: data.count,
        avgRiskScore: data.count > 0 ? Math.round((data.totalRisk / data.count) * 10) / 10 : 0,
      });
    }
  ...
  async getGlobalAnalytics(): Promise<{
    ...
    const avgRiskScore = totalDocuments > 0 ? Math.round((totalRiskScore / totalDocuments) * 10) / 10 : 0;
